...
public class GameManagerService {
    ...
    public Response addPartida(Partida partida) {
        if (partida.getEmail() == null || partida.getDate() == null || partida.getPoints() == 0 || partida.getAvatar() == null) {
            return Response.status(500).build();
        }
        gameManager.addPartida(partida.getEmail(), partida.getDate(), partida.getPoints(), partida.getAvatar());
    ...
    public Response getPartidas() {
        List<Partida> partidas = this.gameManager.getPartidas();
    ...
    @GET
    @ApiOperation(value = "get all Partidas by Points", notes = "Gets all the partidas order by points")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successful", response = Partida.class, responseContainer = "List")
    })
    @Path("/stats/ranking")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPartidasByPoints() {
        List<Partida> partidasByPoints = this.gameManager.getPartidasByPoints();
        GenericEntity<List<Partida>> entity = new GenericEntity<List<Partida>>(partidasByPoints) {
        };
        return Response.status(200).entity(entity).build();
    }
}